...
public class PlayerFPS : NetworkBehaviour {
    ...
    void RpcPlayerFired(uint shooterID, Vector3 impactPos, Vector3 impactRot)
    {
        ...
        //NetworkIdentity.spawned[shooterID].GetComponent<PlayerFPS>().MuzzleFlash();
    }
    ...
    public void Damage(int amount, uint shooterID)
    {
        ...
        {
            Die();
            //NetworkIdentity.spawned[shooterID].GetComponent<PlayerFPS>().Kills++;
            //NetworkIdentity.spawned[shooterID].GetComponent<PlayerFPS>().TargetGotKill();
            
            GetComponent<PlayerFPS>().Kills++;
            GetComponent<PlayerFPS>().TargetGotKill();
        }
    }
